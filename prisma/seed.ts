...
export async function main() {
  ...
  const clinicMemberships: Array<{
    ...
    { clinicName: "Alpha Clinic", userEmail: "alice@example.com", role: "OWNER", color: "#6366f1" },
    { clinicName: "Alpha Clinic", userEmail: "bob@example.com", role: "ADMIN", color: "#22c55e" },
    { clinicName: "Alpha Clinic", userEmail: "carol@example.com", role: "MEMBER", color: "#ec4899" },
    { clinicName: "Beta Clinic", userEmail: "dave@example.com", role: "OWNER", color: "#f97316" },
    { clinicName: "Beta Clinic", userEmail: "eve@example.com", role: "ADMIN", color: "#3b82f6" },
    { clinicName: "Beta Clinic", userEmail: "frank@example.com", role: "MEMBER", color: "#14b8a6" },
  ];
  ...
  for (const membership of clinicMemberships) {
    ...
    await prisma.clinicMember.upsert({
      ...
      update: { userId: user.id, role: membership.role, color: membership.color },
    });
  ...
  // ── Patients ───────────────────────────────────────────────────────────────
  const patientsData = [
    { email: "patient.anna@example.com", phone: "+3610000001", name: "Molnár Anna", birthDate: new Date("1990-02-15T00:00:00Z"), sex: "FEMALE" as const, clinicName: "Alpha Clinic" as const },
    { email: "patient.bela@example.com", phone: "+3610000002", name: "Horváth Béla", birthDate: new Date("1985-06-20T00:00:00Z"), sex: "MALE" as const, clinicName: "Alpha Clinic" as const },
    { email: "patient.csilla@example.com", phone: "+3610000003", name: "Farkas Csilla", birthDate: new Date("2000-11-05T00:00:00Z"), sex: "FEMALE" as const, clinicName: "Alpha Clinic" as const },
    { email: "patient.denes@example.com", phone: "+3610000004", name: "Takács Dénes", birthDate: new Date("1978-09-12T00:00:00Z"), sex: "MALE" as const, clinicName: "Beta Clinic" as const },
    { email: "patient.eva@example.com", phone: "+3610000005", name: "Balogh Éva", birthDate: new Date("1992-01-10T00:00:00Z"), sex: "FEMALE" as const, clinicName: "Beta Clinic" as const },
    { email: "patient.ferenc@example.com", phone: "+3610000006", name: "Papp Ferenc", birthDate: new Date("1989-03-03T00:00:00Z"), sex: "MALE" as const, clinicName: "Beta Clinic" as const },
    { email: "patient.gyorgy@example.com", phone: "+3610000007", name: "Lakatos György", birthDate: new Date("1975-12-25T00:00:00Z"), sex: "MALE" as const, clinicName: "Beta Clinic" as const },
    { email: "patient.hanna@example.com", phone: "+3610000008", name: "Simon Hanna", birthDate: new Date("2001-07-07T00:00:00Z"), sex: "FEMALE" as const, clinicName: "Alpha Clinic" as const },
  ];
  ...
  for (const p of patientsData) {
    ...
    const up = await prisma.patient.upsert({
      where: { email: p.email },
      create: { email: p.email, phone: p.phone, name: p.name, birthDate: p.birthDate, sex: p.sex, clinicId: clinic.id },
      update: { phone: p.phone, name: p.name, birthDate: p.birthDate, sex: p.sex, clinicId: clinic.id },
    });
  ...
  const assignments: Array<{ doctorEmail: string; patientEmail: string }> = [
    { doctorEmail: "alice@example.com", patientEmail: "patient.anna@example.com" },
    { doctorEmail: "alice@example.com", patientEmail: "patient.csilla@example.com" },
    { doctorEmail: "alice@example.com", patientEmail: "patient.hanna@example.com" },
    { doctorEmail: "bob@example.com", patientEmail: "patient.bela@example.com" },
    { doctorEmail: "bob@example.com", patientEmail: "patient.ferenc@example.com" },
    { doctorEmail: "carol@example.com", patientEmail: "patient.csilla@example.com" },
    { doctorEmail: "carol@example.com", patientEmail: "patient.eva@example.com" },
    { doctorEmail: "dave@example.com", patientEmail: "patient.denes@example.com" },
    { doctorEmail: "dave@example.com", patientEmail: "patient.gyorgy@example.com" },
    { doctorEmail: "eve@example.com", patientEmail: "patient.hanna@example.com" },
    { doctorEmail: "frank@example.com", patientEmail: "patient.ferenc@example.com" },
  ];
  ...
  for (const opData of operationsData) {
    const existing = await prisma.operation.findFirst({ where: { name: opData.name } });
    const op = existing ?? (await prisma.operation.create({ data: { name: opData.name } }));
    operations[opData.name] = op as { id: string; name: string };

    // Prices per clinic
    for (const clinic of Object.values(clinics)) {
      const price = clinic.name === "Alpha Clinic" ? opData.alphaPrice : opData.betaPrice;
      await prisma.clinicOperationPrice.upsert({
        where: { clinicId_operationId: { clinicId: clinic.id, operationId: op.id } },
  ...
  for (const patient of Object.values(patients)) {
    await prisma.patientTooth.createMany({
      data: teethCodes.map((toothCode) => ({ patientId: patient.id, toothCode })),
      skipDuplicates: true,
    });
  }
  ...
  const toothStatuses: Array<{
    patientEmail: string;
    toothCode: string;
    status: "CARIES" | "MISSING" | "FILLING" | "CROWN" | "ROOT_CANAL" | "BRIDGE" | "IMPLANT" | "IMPACTED";
  }> = [
    // Anna – has some fillings and a missing tooth
    { patientEmail: "patient.anna@example.com", toothCode: "16", status: "FILLING" },
    { patientEmail: "patient.anna@example.com", toothCode: "26", status: "FILLING" },
    { patientEmail: "patient.anna@example.com", toothCode: "36", status: "MISSING" },
    { patientEmail: "patient.anna@example.com", toothCode: "46", status: "CARIES" },
    // Béla – root canal + crown
    { patientEmail: "patient.bela@example.com", toothCode: "14", status: "ROOT_CANAL" },
    { patientEmail: "patient.bela@example.com", toothCode: "15", status: "CROWN" },
    { patientEmail: "patient.bela@example.com", toothCode: "38", status: "IMPACTED" },
    { patientEmail: "patient.bela@example.com", toothCode: "48", status: "IMPACTED" },
    // Csilla – implant
    { patientEmail: "patient.csilla@example.com", toothCode: "21", status: "IMPLANT" },
    { patientEmail: "patient.csilla@example.com", toothCode: "37", status: "FILLING" },
    // Dénes – bridge + several fillings
    { patientEmail: "patient.denes@example.com", toothCode: "11", status: "BRIDGE" },
    { patientEmail: "patient.denes@example.com", toothCode: "12", status: "BRIDGE" },
    { patientEmail: "patient.denes@example.com", toothCode: "13", status: "BRIDGE" },
    { patientEmail: "patient.denes@example.com", toothCode: "25", status: "FILLING" },
    { patientEmail: "patient.denes@example.com", toothCode: "35", status: "FILLING" },
    { patientEmail: "patient.denes@example.com", toothCode: "45", status: "MISSING" },
    // Éva – caries
    { patientEmail: "patient.eva@example.com", toothCode: "17", status: "CARIES" },
    { patientEmail: "patient.eva@example.com", toothCode: "27", status: "CARIES" },
    // György – many missing teeth (older patient)
    { patientEmail: "patient.gyorgy@example.com", toothCode: "18", status: "MISSING" },
    { patientEmail: "patient.gyorgy@example.com", toothCode: "28", status: "MISSING" },
    { patientEmail: "patient.gyorgy@example.com", toothCode: "38", status: "MISSING" },
    { patientEmail: "patient.gyorgy@example.com", toothCode: "48", status: "MISSING" },
    { patientEmail: "patient.gyorgy@example.com", toothCode: "16", status: "CROWN" },
    { patientEmail: "patient.gyorgy@example.com", toothCode: "26", status: "CROWN" },
    { patientEmail: "patient.gyorgy@example.com", toothCode: "36", status: "ROOT_CANAL" },
  ];
  ...
  for (const ts of toothStatuses) {
    ...
    await prisma.patientTooth.update({
      where: { patientId_toothCode: { patientId: patient.id, toothCode: ts.toothCode } },
  ...
  const buildDate = (dayOffset: number, hour: number, minute: number) =>
    new Date(today.getFullYear(), today.getMonth(), today.getDate() + dayOffset, hour, minute, 0, 0);
  ...
  const appointmentPlan: Array<{
    ...
    // ── Past appointments (history) ────────
    {
      ...
      dayOffset: -14,
      startHour: 9, startMinute: 0, durationMinutes: 45,
      toothOps: [
    ...
    },
    {
      ...
      toothOps: [
        { toothCode: "14", operationName: "Gyökérkezelés" },
      ],
    },
    {
      doctorEmail: "bob@example.com",
      ...
      toothOps: [
        { toothCode: "15", operationName: "Korona készítés" },
      ],
    },
    {
      doctorEmail: "alice@example.com",
      ...
      dayOffset: -7,
      startHour: 14, startMinute: 0, durationMinutes: 90,
      toothOps: [
        { toothCode: "21", operationName: "Implantátum beültetés" },
      ],
    },
    {
      ...
      dayOffset: -21,
      startHour: 9, startMinute: 30, durationMinutes: 60,
      toothOps: [
    ...
    // ── Today's appointments ────────
    {
      ...
      toothOps: [
        { toothCode: "46", operationName: "Kontroll vizsgálat" },
      ],
    },
    {
      doctorEmail: "bob@example.com",
    ...
    },
    {
      ...
      dayOffset: 0,
      startHour: 11, startMinute: 0, durationMinutes: 45,
      toothOps: [
    ...
    },
    {
      ...
      dayOffset: 1,
      startHour: 14, startMinute: 0, durationMinutes: 20,
      toothOps: [
        { toothCode: "38", operationName: "Panoráma röntgen" },
      ],
    },
    {
      doctorEmail: "alice@example.com",
      ...
      startHour: 10, startMinute: 0, durationMinutes: 60,
      toothOps: [
        { toothCode: "36", operationName: "Foghúzás" },
      ],
    },
    {
      doctorEmail: "carol@example.com",
      ...
      dayOffset: 2,
      startHour: 15, startMinute: 0, durationMinutes: 45,
      toothOps: [
        { toothCode: "37", operationName: "Parodontológiai kezelés" },
      ],
    },
    {
      doctorEmail: "dave@example.com",
  ...
  for (const plan of appointmentPlan) {
    ...
    const membership = clinicMemberships.find((m) => m.userEmail === plan.doctorEmail);
    ...
    const endTime = new Date(start.getTime() + plan.durationMinutes * 60 * 1000);
